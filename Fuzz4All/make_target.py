...
def make_target(kwargs: Dict[str, Any]) -> Target:
    ...
    if language == "cpp":
        ...
        from Fuzz4All.target.Cirq.CIRQ import CirqTarget

        return CirqTarget(**kwargs)
    elif language == "sympy":
        from Fuzz4All.target.SYMPY.SYMPY import SymPyTarget

        return SymPyTarget(**kwargs)
    elif language == "smt2":
...
def make_target_with_config(config_dict: Dict[str, Any]) -> Target:
    ...
    elif target["language"] == "cirq":
        from Fuzz4All.target.Cirq.CIRQ import CirqTarget

        return CirqTarget(**target_compat_dict)
    elif target["language"] == "sympy":
        from Fuzz4All.target.SYMPY.SYMPY import SymPyTarget

        return SymPyTarget(**target_compat_dict)
