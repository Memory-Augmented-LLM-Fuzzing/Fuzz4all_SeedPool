...
from Fuzz4All.seed_pool import SeedPool
...
def fuzz(
    ...
    pool_dir = os.path.join(output_folder, "seed_pool")
    seed_pool = SeedPool(pool_dir)
    target.seed_pool = seed_pool  # Make pool accessible to target
    ...
    with Progress(
        ...
        while (
            ...
            for index, fo in enumerate(fos):
                ...
                if otf:
                    validation_result = target.validate_individual(file_name)

                    # Handle both old (2-tuple) and new (4-tuple) return formats
                    if len(validation_result) == 2:
                        f_result, message = validation_result
                        coverage_delta = 0.0
                        rare_edges = []
                    else:
                        f_result, message, coverage_delta, rare_edges = validation_result

                    target.parse_validation_message(f_result, message, file_name)
                    prev.append((f_result, fo))

                    # Add to seed pool on multiple criteria
                    should_save = (
                        f_result == FResult.ERROR  # Bug found
                        or coverage_delta > 0.5  # New coverage > 0.5%
                        or len(rare_edges) > 0  # Rare edges triggered
                    )
                    ...
                    if should_save:
                        try:
                            ...
                            seed_pool.add_seed(
                                ...
                                edges_triggered=rare_edges,
                            )
                        except Exception as e:
                            target.m_logger.logo(
                                f"Error adding seed to pool: {e}",
                                level="ERROR"
                            )
